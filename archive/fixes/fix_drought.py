...
for commune_db in communes_db:
    ...
    if commune_json not in data:
        print(f"Pas trouve dans JSON: {commune_db}")
        continue
    for sc_label, sc_key in sc_map.items():
        ...
        mean_p = sum(all_totals) / len(all_totals)
        std_p = (sum((x-mean_p)**2 for x in all_totals)/len(all_totals))**0.5
        for year, total in precip_by_year.items():
            spi = (total - mean_p) / std_p if std_p > 0 else 0
            drought = max(0, min(1, (mean_p - total) / mean_p)) if mean_p > 0 else 0
