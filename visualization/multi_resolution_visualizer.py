...
class MultiResolutionVisualizer:
    ...
    def plot_daily(
        ...
        ax.fill_between(
            dates, values - conf * 3, values + conf * 3, alpha=0.1, color="#1976D2"
        )
    ...
    def plot_annual(
        ...
        p = np.poly1d(z)
        ax.plot(
            np.linspace(0, len(df) - 1, 200),
            p(np.linspace(0, len(df) - 1, 200)),
            "r--",
            linewidth=2,
            label="Tendance",
        )
    ...
    def dashboard(
        ...
        if not daily.empty:
            ...
            ax0.set_title(
                f"🌡️  Température journalière — {commune} {year}",
                fontweight="bold",
            )
            ax0.set_ylabel("°C")
        ...
        fig.suptitle(
            f"Tableau de bord climatique — {commune} {year}",
            fontsize=15,
            fontweight="bold",
        )
        if save:
    ...
    def risk_map(
        self, communes_data: dict, save: str | None = None
    ) -> folium.Map:
        m = folium.Map(
            location=[14.5, -14.5], zoom_start=6, tiles="OpenStreetMap"
        )
        for name, info in communes_data.items():
            risk = info.get("risk_level", "MEDIUM")
            color = RISK_COLORS.get(risk, "#999")
            popup = (
                f"<b>{name}</b><br>Risque: {risk}<br>"
                f"Temp: {info.get('temp', '-')}°C"
            )
            folium.CircleMarker(
